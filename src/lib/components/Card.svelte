<script lang="ts">
  ...
  let {
    ...
    disabled,
    onclick,
  }: { card: Card; disabled: boolean; onclick: () => void } = $props();
  ...
  let cardClasses = $derived(
    [
      ...
      card.matched && !justMatched
        ? "opacity-30 scale-85 pointer-events-none"
        : "",
...
>
  ...
  <div
    class="absolute inset-0 rounded-xl backface-hidden flex items-center justify-center bg-gradient-to-br from-indigo-500 to-purple-600 border-2 border-indigo-400 shadow-lg hover:shadow-xl transition-shadow {card.matched
      ? 'hidden'
      : ''}"
  >
  ...
  <div
    class="card-front absolute inset-0 rounded-xl backface-hidden [transform:rotateY(180deg)] flex items-center justify-center bg-white border-2 border-gray-200 shadow-lg {justMatched
      ? 'card-front-glow'
      : ''}"
  >
    {#if imageSrc}
